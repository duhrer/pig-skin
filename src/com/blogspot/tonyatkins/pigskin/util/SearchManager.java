...
public class SearchManager {
	...
	public SearchManager(Context context) {
		...
		dictionary = preferences.getString(Constants.DICTIONARY_PREF, Constants.DEFAULT_DICTIONARY);
	}
							...
							for (int e = 0; e < 7; e++) {
								...
								for (int f = 0; f < 7; f++) {
									...
									for (int g = 0; g < 7; g++) {
										...
										char[] charData = { searchString.charAt(a), searchString.charAt(b),
												searchString.charAt(c), searchString.charAt(d), searchString.charAt(e),
												searchString.charAt(f), searchString.charAt(g) };
...
//			validAnagrams = ListUtils.intersection(anagrams, validWords);
		...
		if (prefixWords == null) {
			...
			try {
				ZipInputStream zin = new ZipInputStream(context.getAssets()
						.open("dictionary/" + dictionary + "/" + "words_" + prefix + ".zip"));
				ZipEntry ze;
				while ((ze = zin.getNextEntry()) != null) {
					if (ze.getName().equals("words_" + prefix + ".txt")) {
	...
	public boolean isValidWord(String wordToValidate) {
		...
		for (String word : letterWords) {
			if (word.toLowerCase().equals(wordToValidate.toLowerCase())) {
				return true;
			}
		}
